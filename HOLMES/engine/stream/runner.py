...
class StreamingStats:
    ...
    taint_pruned_entity_count: int = 0
    taint_pruned_version_node_count: int = 0
...
class StreamingEngine:
    def __init__(
        ...
        graph_gc_max_every_events_raw = os.getenv("HOLMES_GRAPH_GC_MAX_EVERY_EVENTS", "").strip()
        if graph_gc_max_every_events_raw:
            try:
                self.graph_gc_max_every_events = max(self.graph_gc_every_events, int(graph_gc_max_every_events_raw))
            except ValueError:
                self.graph_gc_max_every_events = max(self.graph_gc_every_events, self.graph_gc_every_events * 8)
        else:
            self.graph_gc_max_every_events = max(self.graph_gc_every_events, self.graph_gc_every_events * 8)
        pressure_ratio_raw = os.getenv("HOLMES_GRAPH_GC_CAP_PRESSURE_RATIO", "0.98").strip()
        try:
            self.graph_gc_cap_pressure_ratio = min(1.0, max(0.0, float(pressure_ratio_raw)))
        except ValueError:
            self.graph_gc_cap_pressure_ratio = 0.98
        low_watermark_ratio_raw = os.getenv("HOLMES_GRAPH_PRUNE_LOW_WATERMARK_RATIO", "0.90").strip()
        try:
            self.graph_prune_low_watermark_ratio = min(1.0, max(0.0, float(low_watermark_ratio_raw)))
        except ValueError:
            self.graph_prune_low_watermark_ratio = 0.90
        self._current_graph_gc_every_events = int(self.graph_gc_every_events)
        self._graph_gc_cap_trigger_total = 0
        taint_retention_days_raw = os.getenv("HOLMES_TAINT_RETENTION_DAYS", "").strip()
        if taint_retention_days_raw:
            try:
                self.taint_retention_days = max(0, int(taint_retention_days_raw))
            except ValueError:
                self.taint_retention_days = min(self.graph_retention_days, 7)
        else:
            self.taint_retention_days = min(self.graph_retention_days, 7)
        ...
        flush_policy_raw = os.getenv("HOLMES_ONLINE_GRAPH_EDGE_FLUSH_POLICY", "immediate").strip().lower()
        if flush_policy_raw not in {"immediate", "match_driven"}:
            flush_policy_raw = "immediate"
        self.online_graph_edge_flush_policy = flush_policy_raw
        flush_max_pending_raw = os.getenv("HOLMES_ONLINE_GRAPH_EDGE_FLUSH_MAX_PENDING_EDGES", "2048").strip()
        try:
            self.online_graph_edge_flush_max_pending_edges = max(1, int(flush_max_pending_raw))
        except ValueError:
            self.online_graph_edge_flush_max_pending_edges = 2048
        ...
        native_flush_authoritative_raw = os.getenv("HOLMES_NATIVE_ONLINE_FLUSH_AUTHORITATIVE", "0").strip().lower()
        self.native_online_flush_authoritative_enabled = native_flush_authoritative_raw not in {
            "",
            "0",
            "false",
            "no",
        ...
        self._native_online_fallback_activation_total = 0
        self._native_online_fallback_mutation_failure_total = 0
        self._native_online_fallback_read_mismatch_total = 0
        self._native_online_fallback_first_reason: str | None = None
        ...
        graph_max_version_nodes_raw = os.getenv("HOLMES_GRAPH_MAX_VERSION_NODES", "0").strip()
        graph_max_edges_raw = os.getenv("HOLMES_GRAPH_MAX_EDGES", "0").strip()
        try:
            self.graph_max_version_nodes = max(0, int(graph_max_version_nodes_raw))
        except ValueError:
            self.graph_max_version_nodes = 0
        try:
            self.graph_max_edges = max(0, int(graph_max_edges_raw))
        except ValueError:
            self.graph_max_edges = 0
        ...
        self._earliest_event_dt: datetime | None = None
        ...
        self._hsg_update_call_count = 0
        self._hsg_update_changed_match_count = 0
        self._hsg_update_changed_edge_count = 0
        ...
        self._online_graph_edge_flush_python_apply_time_seconds = 0.0
        self._online_graph_edge_flush_native_add_time_seconds = 0.0
        self._online_graph_edge_flush_flush_call_time_seconds = 0.0
        self._online_graph_edge_flush_call_count = 0
        self._online_graph_edge_flush_skipped_count = 0
        self._online_graph_edge_flush_forced_count = 0
        self._online_graph_edge_flush_match_driven_count = 0
        self._online_graph_edge_flush_threshold_count = 0
        self._online_graph_edge_flush_native_authoritative_count = 0
        self._online_match_add_native_authoritative_count = 0
        self._online_match_remove_native_authoritative_count = 0
    ...
    def _build_performance_metrics(self) -> dict[str, Any]:
        self.graph.flush_pending_native_graph_shadow()
        ...
        graph_semantic_time_seconds = float(
            self.graph._event_semantic_extract_time_seconds + self.graph._semantic_register_time_seconds  # noqa: SLF001
        )
        graph_entity_identity_time_seconds = float(
            self.graph._native_record_graph_event_time_seconds  # noqa: SLF001
            + self.graph._memory_sync_time_seconds  # noqa: SLF001
            + self.graph._ensure_entity_time_seconds  # noqa: SLF001
            + self.graph._flow_direction_time_seconds  # noqa: SLF001
        )
        graph_versioning_time_seconds = float(
            self.graph._changed_entities_time_seconds  # noqa: SLF001
            + self.graph._prev_by_entity_snapshot_time_seconds  # noqa: SLF001
            + self.graph._changed_entities_sort_time_seconds  # noqa: SLF001
            + self.graph._bump_entity_new_version_time_seconds  # noqa: SLF001
        )
        graph_edge_bookkeeping_time_seconds = float(
            self.graph._bump_entity_prev_version_link_time_seconds  # noqa: SLF001
            + self.graph._runtime_edge_append_time_seconds  # noqa: SLF001
            + self.graph._provenance_edge_append_time_seconds  # noqa: SLF001
            + self.graph._edge_hook_time_seconds  # noqa: SLF001
            + self.graph._ancestor_delta_propagation_time_seconds  # noqa: SLF001
            + self.graph._flow_link_time_seconds  # noqa: SLF001
            + self.graph._memory_transition_link_time_seconds  # noqa: SLF001
            + self.graph._typed_adjacency_add_time_seconds  # noqa: SLF001
            + self.graph._path_factor_cache_clear_time_seconds  # noqa: SLF001
            + self.graph._process_parent_update_time_seconds  # noqa: SLF001
            + self.graph._add_event_return_payload_time_seconds  # noqa: SLF001
        )
        graph_lookup_meta_time_seconds = float(
            self.graph._node_meta_time_seconds  # noqa: SLF001
            + self.graph._current_version_lookup_time_seconds  # noqa: SLF001
            + self.graph._semantic_current_version_lookup_time_seconds  # noqa: SLF001
        )
        graph_event_prep_time_seconds = float(
            self.graph._event_ts_parse_time_seconds  # noqa: SLF001
            + self.graph._entity_last_seen_update_time_seconds  # noqa: SLF001
        )
        graph_local_bookkeeping_time_seconds = float(
            self.graph._memory_semantic_rewrite_time_seconds  # noqa: SLF001
            + self.graph._changed_entities_finalize_time_seconds  # noqa: SLF001
            + self.graph._event_endpoint_resolve_time_seconds  # noqa: SLF001
        )
        graph_runner_overhead_time_seconds = float(self._online_graph_edge_flush_time_seconds)
        graph_core_time_seconds = max(float(self._graph_add_time_seconds) - graph_runner_overhead_time_seconds, 0.0)
        graph_add_accounted_time_seconds = (
            graph_semantic_time_seconds
            + graph_entity_identity_time_seconds
            + graph_versioning_time_seconds
            + graph_edge_bookkeeping_time_seconds
            + graph_lookup_meta_time_seconds
            + graph_event_prep_time_seconds
            + graph_local_bookkeeping_time_seconds
        )
        graph_add_residual_time_seconds = max(float(self._graph_add_time_seconds) - graph_add_accounted_time_seconds, 0.0)
        native_record_payload_encode_time_seconds = float(
            getattr(self.native_backend, "graph_record_payload_encode_time_seconds", 0.0)
        )
        native_record_ffi_call_time_seconds = float(
            getattr(self.native_backend, "graph_record_ffi_call_time_seconds", 0.0)
        )
        native_record_event_count = int(getattr(self.native_backend, "graph_record_event_count", 0))
        native_record_ffi_call_count = int(getattr(self.native_backend, "graph_record_ffi_call_count", 0))
        native_record_overhead_time_seconds = max(
            float(self.graph._native_record_graph_event_time_seconds)  # noqa: SLF001
            - native_record_payload_encode_time_seconds
            - native_record_ffi_call_time_seconds,
            0.0,
        )
        ...
        native_online_node_count, native_online_depth_cutoff_total, native_online_fanout_cutoff_total, native_online_max_depth = (
            self.native_backend.online_index_stats() if self._native_online_shadow_enabled() else (0, 0, 0, 0)
        )
        native_graph_node_count, native_graph_edge_count = (
            self.native_backend.graph_stats() if self.native_backend.available else (0, 0)
        )
        (
            native_graph_current_version_check_total,
            native_graph_current_version_mismatch_total,
            native_graph_prune_preview_check_total,
            native_graph_prune_preview_mismatch_total,
        ) = self.graph.native_graph_shadow_stats()
        return {
            "events_per_second": float(self.stats.events) / elapsed_seconds,
            "rolling_events_per_second_60s": self._rolling_events_per_second(60.0),
            "rolling_events_per_second_300s": self._rolling_events_per_second(300.0),
            ...
            "graph_add_core_time_seconds": graph_core_time_seconds,
            "graph_add_core_avg_ms_per_event": (graph_core_time_seconds * 1000.0 / float(self.stats.events))
            if self.stats.events
            else 0.0,
            "graph_add_core_share": (graph_core_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_runner_overhead_time_seconds": graph_runner_overhead_time_seconds,
            "graph_add_runner_overhead_avg_ms_per_event": (
                graph_runner_overhead_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_add_runner_overhead_share": (graph_runner_overhead_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_semantic_time_seconds": graph_semantic_time_seconds,
            "graph_add_semantic_avg_ms_per_event": (graph_semantic_time_seconds * 1000.0 / float(self.stats.events))
            if self.stats.events
            else 0.0,
            "graph_add_semantic_share": (graph_semantic_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_entity_identity_time_seconds": graph_entity_identity_time_seconds,
            "graph_add_entity_identity_avg_ms_per_event": (
                graph_entity_identity_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_add_entity_identity_share": (graph_entity_identity_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_versioning_time_seconds": graph_versioning_time_seconds,
            "graph_add_versioning_avg_ms_per_event": (graph_versioning_time_seconds * 1000.0 / float(self.stats.events))
            if self.stats.events
            else 0.0,
            "graph_add_versioning_share": (graph_versioning_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_edge_bookkeeping_time_seconds": graph_edge_bookkeeping_time_seconds,
            "graph_add_edge_bookkeeping_avg_ms_per_event": (
                graph_edge_bookkeeping_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_add_edge_bookkeeping_share": (graph_edge_bookkeeping_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_lookup_meta_time_seconds": graph_lookup_meta_time_seconds,
            "graph_add_lookup_meta_avg_ms_per_event": (
                graph_lookup_meta_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_add_lookup_meta_share": (graph_lookup_meta_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_event_prep_time_seconds": graph_event_prep_time_seconds,
            "graph_add_event_prep_avg_ms_per_event": (graph_event_prep_time_seconds * 1000.0 / float(self.stats.events))
            if self.stats.events
            else 0.0,
            "graph_add_event_prep_share": (graph_event_prep_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_local_bookkeeping_time_seconds": graph_local_bookkeeping_time_seconds,
            "graph_add_local_bookkeeping_avg_ms_per_event": (
                graph_local_bookkeeping_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_add_local_bookkeeping_share": (graph_local_bookkeeping_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_accounted_time_seconds": graph_add_accounted_time_seconds,
            "graph_add_accounted_share": (graph_add_accounted_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            "graph_add_residual_time_seconds": graph_add_residual_time_seconds,
            "graph_add_residual_avg_ms_per_event": (graph_add_residual_time_seconds * 1000.0 / float(self.stats.events))
            if self.stats.events
            else 0.0,
            "graph_add_residual_share": (graph_add_residual_time_seconds / float(self._graph_add_time_seconds))
            if self._graph_add_time_seconds > 0.0
            else 0.0,
            ...
            "graph_native_record_graph_event_time_seconds": float(self.graph._native_record_graph_event_time_seconds),  # noqa: SLF001
            "graph_native_record_graph_event_avg_ms_per_event": (
                float(self.graph._native_record_graph_event_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_native_record_payload_encode_time_seconds": native_record_payload_encode_time_seconds,
            "graph_native_record_payload_encode_avg_ms_per_event": (
                native_record_payload_encode_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_native_record_ffi_call_time_seconds": native_record_ffi_call_time_seconds,
            "graph_native_record_ffi_call_avg_ms_per_event": (
                native_record_ffi_call_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_native_record_overhead_time_seconds": native_record_overhead_time_seconds,
            "graph_native_record_overhead_avg_ms_per_event": (
                native_record_overhead_time_seconds * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_native_record_event_count": native_record_event_count,
            "graph_native_record_ffi_call_count": native_record_ffi_call_count,
            "graph_native_record_avg_events_per_ffi_call": (
                float(native_record_event_count) / float(native_record_ffi_call_count)
            )
            if native_record_ffi_call_count > 0
            else 0.0,
            ...
            "graph_event_ts_parse_time_seconds": float(self.graph._event_ts_parse_time_seconds),  # noqa: SLF001
            "graph_event_ts_parse_avg_ms_per_event": (
                float(self.graph._event_ts_parse_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_entity_last_seen_update_time_seconds": float(self.graph._entity_last_seen_update_time_seconds),  # noqa: SLF001
            "graph_entity_last_seen_update_avg_ms_per_event": (
                float(self.graph._entity_last_seen_update_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_memory_semantic_rewrite_time_seconds": float(self.graph._memory_semantic_rewrite_time_seconds),  # noqa: SLF001
            "graph_memory_semantic_rewrite_avg_ms_per_event": (
                float(self.graph._memory_semantic_rewrite_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            ...
            "graph_changed_entities_finalize_time_seconds": float(self.graph._changed_entities_finalize_time_seconds),  # noqa: SLF001
            "graph_changed_entities_finalize_avg_ms_per_event": (
                float(self.graph._changed_entities_finalize_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            ...
            "graph_prev_by_entity_snapshot_time_seconds": float(self.graph._prev_by_entity_snapshot_time_seconds),  # noqa: SLF001
            "graph_prev_by_entity_snapshot_avg_ms_per_event": (
                float(self.graph._prev_by_entity_snapshot_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_changed_entities_sort_time_seconds": float(self.graph._changed_entities_sort_time_seconds),  # noqa: SLF001
            "graph_changed_entities_sort_avg_ms_per_event": (
                float(self.graph._changed_entities_sort_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_bump_entity_new_version_time_seconds": float(self.graph._bump_entity_new_version_time_seconds),  # noqa: SLF001
            "graph_bump_entity_new_version_avg_ms_per_event": (
                float(self.graph._bump_entity_new_version_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_bump_entity_prev_version_link_time_seconds": float(self.graph._bump_entity_prev_version_link_time_seconds),  # noqa: SLF001
            "graph_bump_entity_prev_version_link_avg_ms_per_event": (
                float(self.graph._bump_entity_prev_version_link_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            ...
            "graph_ancestor_delta_propagation_time_seconds": float(self.graph._ancestor_delta_propagation_time_seconds),  # noqa: SLF001
            "graph_ancestor_delta_propagation_avg_ms_per_event": (
                float(self.graph._ancestor_delta_propagation_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_process_parent_update_time_seconds": float(self.graph._process_parent_update_time_seconds),  # noqa: SLF001
            "graph_process_parent_update_avg_ms_per_event": (
                float(self.graph._process_parent_update_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_add_event_return_payload_time_seconds": float(self.graph._add_event_return_payload_time_seconds),  # noqa: SLF001
            "graph_add_event_return_payload_avg_ms_per_event": (
                float(self.graph._add_event_return_payload_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "graph_event_endpoint_resolve_time_seconds": float(self.graph._event_endpoint_resolve_time_seconds),  # noqa: SLF001
            "graph_event_endpoint_resolve_avg_ms_per_event": (
                float(self.graph._event_endpoint_resolve_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            ...
            "online_graph_edge_flush_python_apply_time_seconds": float(
                self._online_graph_edge_flush_python_apply_time_seconds
            ),
            "online_graph_edge_flush_python_apply_avg_ms_per_event": (
                float(self._online_graph_edge_flush_python_apply_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_graph_edge_flush_native_add_time_seconds": float(
                self._online_graph_edge_flush_native_add_time_seconds
            ),
            "online_graph_edge_flush_native_add_avg_ms_per_event": (
                float(self._online_graph_edge_flush_native_add_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_graph_edge_flush_flush_call_time_seconds": float(
                self._online_graph_edge_flush_flush_call_time_seconds
            ),
            "online_graph_edge_flush_flush_call_avg_ms_per_event": (
                float(self._online_graph_edge_flush_flush_call_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_graph_edge_flush_call_count": int(self._online_graph_edge_flush_call_count),
            "online_graph_edge_flush_skipped_count": int(self._online_graph_edge_flush_skipped_count),
            "online_graph_edge_flush_forced_count": int(self._online_graph_edge_flush_forced_count),
            "online_graph_edge_flush_match_driven_count": int(self._online_graph_edge_flush_match_driven_count),
            "online_graph_edge_flush_threshold_count": int(self._online_graph_edge_flush_threshold_count),
            "online_graph_edge_flush_native_authoritative_count": int(
                self._online_graph_edge_flush_native_authoritative_count
            ),
            "online_match_add_native_authoritative_count": int(self._online_match_add_native_authoritative_count),
            "online_match_remove_native_authoritative_count": int(self._online_match_remove_native_authoritative_count),
            "online_graph_edge_flush_pending_count": int(len(self._pending_online_graph_edges)),
            "online_graph_edge_flush_policy_match_driven_enabled": int(
                self.online_graph_edge_flush_policy == "match_driven"
            ),
            "online_graph_edge_flush_max_pending_edges": int(self.online_graph_edge_flush_max_pending_edges),
            "online_index_flush_pending_edges_total_time_seconds": float(
                self.online_index.flush_pending_edges_total_time_seconds
            ),
            "online_index_flush_pending_edges_total_avg_ms_per_event": (
                float(self.online_index.flush_pending_edges_total_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_index_flush_pending_edges_loop_time_seconds": float(
                self.online_index.flush_pending_edges_loop_time_seconds
            ),
            "online_index_flush_pending_edges_loop_avg_ms_per_event": (
                float(self.online_index.flush_pending_edges_loop_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_index_flush_pending_edges_propagate_time_seconds": float(
                self.online_index.flush_pending_edges_propagate_time_seconds
            ),
            "online_index_flush_pending_edges_propagate_avg_ms_per_event": (
                float(self.online_index.flush_pending_edges_propagate_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_index_flush_pending_edges_batch_count": int(self.online_index.flush_pending_edges_batch_count),
            "online_index_flush_pending_edges_edge_count": int(self.online_index.flush_pending_edges_edge_count),
            "online_index_propagate_across_new_edge_seed_merge_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_seconds
            ),
            "online_index_propagate_across_new_edge_seed_merge_avg_ms_per_event": (
                float(self.online_index.propagate_across_new_edge_seed_merge_time_seconds)
                * 1000.0
                / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_index_propagate_across_new_edge_downstream_time_seconds": float(
                self.online_index.propagate_across_new_edge_downstream_time_seconds
            ),
            "online_index_propagate_across_new_edge_downstream_avg_ms_per_event": (
                float(self.online_index.propagate_across_new_edge_downstream_time_seconds)
                * 1000.0
                / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_index_propagate_across_new_edge_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_total
            ),
            "online_index_propagate_across_new_edge_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_total
            ),
            "online_index_propagate_across_new_edge_changed_match_ratio": (
                float(self.online_index.propagate_across_new_edge_changed_match_total)
                / float(self.online_index.propagate_across_new_edge_scanned_match_total)
            )
            if int(self.online_index.propagate_across_new_edge_scanned_match_total) > 0
            else 0.0,
            "online_index_propagate_across_new_edge_empty_src_count": int(
                self.online_index.propagate_across_new_edge_empty_src_count
            ),
            "online_index_propagate_across_new_edge_no_change_count": int(
                self.online_index.propagate_across_new_edge_no_change_count
            ),
            "online_index_seed_merge_data_flow_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_edge_type.get("data_flow", 0.0)
            ),
            "online_index_seed_merge_data_flow_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_edge_type.get("data_flow", 0)
            ),
            "online_index_seed_merge_data_flow_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_edge_type.get("data_flow", 0)
            ),
            "online_index_seed_merge_version_transition_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_edge_type.get("version_transition", 0.0)
            ),
            "online_index_seed_merge_version_transition_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_edge_type.get("version_transition", 0)
            ),
            "online_index_seed_merge_version_transition_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_edge_type.get("version_transition", 0)
            ),
            "online_index_seed_merge_bucket_1_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_bucket.get("1", 0.0)
            ),
            "online_index_seed_merge_bucket_1_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_bucket.get("1", 0)
            ),
            "online_index_seed_merge_bucket_1_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_bucket.get("1", 0)
            ),
            "online_index_seed_merge_bucket_2_4_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_bucket.get("2_4", 0.0)
            ),
            "online_index_seed_merge_bucket_2_4_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_bucket.get("2_4", 0)
            ),
            "online_index_seed_merge_bucket_2_4_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_bucket.get("2_4", 0)
            ),
            "online_index_seed_merge_bucket_5_16_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_bucket.get("5_16", 0.0)
            ),
            "online_index_seed_merge_bucket_5_16_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_bucket.get("5_16", 0)
            ),
            "online_index_seed_merge_bucket_5_16_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_bucket.get("5_16", 0)
            ),
            "online_index_seed_merge_bucket_17_64_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_bucket.get("17_64", 0.0)
            ),
            "online_index_seed_merge_bucket_17_64_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_bucket.get("17_64", 0)
            ),
            "online_index_seed_merge_bucket_17_64_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_bucket.get("17_64", 0)
            ),
            "online_index_seed_merge_bucket_65_plus_time_seconds": float(
                self.online_index.propagate_across_new_edge_seed_merge_time_by_bucket.get("65_plus", 0.0)
            ),
            "online_index_seed_merge_bucket_65_plus_scanned_match_total": int(
                self.online_index.propagate_across_new_edge_scanned_match_by_bucket.get("65_plus", 0)
            ),
            "online_index_seed_merge_bucket_65_plus_changed_match_total": int(
                self.online_index.propagate_across_new_edge_changed_match_by_bucket.get("65_plus", 0)
            ),
            "online_index_seed_merge_empty_dst_fast_path_count": int(
                self.online_index.propagate_across_new_edge_empty_dst_fast_path_count
            ),
            "online_index_seed_merge_empty_dst_fast_path_time_seconds": float(
                self.online_index.propagate_across_new_edge_empty_dst_fast_path_time_seconds
            ),
            "online_index_seed_merge_data_flow_precheck_time_seconds": float(
                self.online_index.propagate_across_new_edge_data_flow_precheck_time_seconds
            ),
            "online_index_seed_merge_data_flow_precheck_count": int(
                self.online_index.propagate_across_new_edge_data_flow_precheck_count
            ),
            "online_index_seed_merge_data_flow_precheck_hit_count": int(
                self.online_index.propagate_across_new_edge_data_flow_precheck_hit_count
            ),
            "online_index_seed_merge_data_flow_precheck_hit_ratio": (
                float(self.online_index.propagate_across_new_edge_data_flow_precheck_hit_count)
                / float(self.online_index.propagate_across_new_edge_data_flow_precheck_count)
            )
            if int(self.online_index.propagate_across_new_edge_data_flow_precheck_count) > 0
            else 0.0,
            "online_index_propagate_delta_time_seconds": float(self.online_index.propagate_delta_time_seconds),
            "online_index_propagate_delta_avg_ms_per_event": (
                float(self.online_index.propagate_delta_time_seconds) * 1000.0 / float(self.stats.events)
            )
            if self.stats.events
            else 0.0,
            "online_index_propagate_delta_queue_pop_count": int(self.online_index.propagate_delta_queue_pop_count),
            "online_index_propagate_delta_edge_visit_count": int(self.online_index.propagate_delta_edge_visit_count),
            "online_index_propagate_delta_changed_enqueue_count": int(
                self.online_index.propagate_delta_changed_enqueue_count
            ),
            ...
            "native_online_flush_authoritative_enabled": int(bool(self.native_online_flush_authoritative_enabled)),
            "native_graph_shadow_enabled": int(bool(self.graph._native_graph_shadow_enabled)),  # noqa: SLF001
            ...
            "native_online_shadow_enabled": int(bool(self._native_online_shadow_enabled())),
            "native_graph_node_count": int(native_graph_node_count),
            "native_graph_edge_count": int(native_graph_edge_count),
            "native_graph_current_version_check_total": int(native_graph_current_version_check_total),
            "native_graph_current_version_mismatch_total": int(native_graph_current_version_mismatch_total),
            "native_graph_prune_preview_check_total": int(native_graph_prune_preview_check_total),
            "native_graph_prune_preview_mismatch_total": int(native_graph_prune_preview_mismatch_total),
            ...
            "native_online_fallback_activation_total": int(self._native_online_fallback_activation_total),
            "native_online_fallback_mutation_failure_total": int(
                self._native_online_fallback_mutation_failure_total
            ),
            "native_online_fallback_read_mismatch_total": int(self._native_online_fallback_read_mismatch_total),
            "native_online_fallback_first_reason": self._native_online_fallback_first_reason or "",
            ...
            "hsg_update_call_count": int(self._hsg_update_call_count),
            "hsg_update_changed_match_count": int(self._hsg_update_changed_match_count),
            "hsg_update_changed_edge_count": int(self._hsg_update_changed_edge_count),
            "hsg_update_avg_changed_matches_per_call": (
                float(self._hsg_update_changed_match_count) / float(self._hsg_update_call_count)
            )
            if self._hsg_update_call_count
            else 0.0,
            "hsg_update_avg_changed_edges_per_call": (
                float(self._hsg_update_changed_edge_count) / float(self._hsg_update_call_count)
            )
            if self._hsg_update_call_count
            else 0.0,
            "graph_gc_time_seconds": float(self._graph_gc_time_seconds),
            "graph_gc_every_events_configured": int(self.graph_gc_every_events),
            "graph_gc_current_every_events": int(self._current_graph_gc_every_events),
            "graph_gc_max_every_events": int(self.graph_gc_max_every_events),
            "graph_gc_cap_pressure_ratio": float(self.graph_gc_cap_pressure_ratio),
            "graph_prune_low_watermark_ratio": float(self.graph_prune_low_watermark_ratio),
            "graph_gc_cap_trigger_total": int(self._graph_gc_cap_trigger_total),
            "process_current_rss_bytes": int(current_rss_bytes),
            "process_current_rss_mb": float(current_rss_bytes / (1024.0 * 1024.0)),
            "process_peak_rss_bytes": int(peak_rss_bytes),
            "process_peak_rss_mb": float(peak_rss_bytes / (1024.0 * 1024.0)),
            ...
            "native_graph_shadow_enabled": int(bool(self.graph._native_graph_shadow_enabled)),  # noqa: SLF001
            "native_online_shadow_enabled": int(bool(self._native_online_shadow_enabled())),
            "native_graph_current_version_check_total": native_graph_current_version_check_total,
            "native_graph_current_version_mismatch_total": native_graph_current_version_mismatch_total,
            "native_graph_prune_preview_check_total": native_graph_prune_preview_check_total,
            "native_graph_prune_preview_mismatch_total": native_graph_prune_preview_mismatch_total,
    ...
    def _record_throughput_sample(self) -> None:
        now = time.perf_counter()
        self._throughput_samples.append((now, int(self.stats.events)))
        cutoff = now - 300.0
        while len(self._throughput_samples) > 1 and self._throughput_samples[0][0] < cutoff:
            self._throughput_samples.popleft()

    def _rolling_events_per_second(self, window_seconds: float) -> float:
        if not self._throughput_samples:
            return 0.0
        now = time.perf_counter()
        latest_ts, latest_events = self._throughput_samples[-1]
        cutoff = now - float(window_seconds)
        baseline_ts = latest_ts
        baseline_events = latest_events
        for sample_ts, sample_events in reversed(self._throughput_samples):
            baseline_ts = sample_ts
            baseline_events = sample_events
            if sample_ts <= cutoff:
                break
        elapsed = max(latest_ts - baseline_ts, 1e-9)
        delta_events = max(latest_events - baseline_events, 0)
        return float(delta_events) / elapsed
    ...
    def _add_active_match_to_online_state(self, match: TTPMatch, *, add_to_online_index: bool) -> bool:
        ...
        if add_to_online_index:
            ...
            native_authoritative = self._native_online_match_authoritative_enabled_for_run()
            native_failed = False
            for node_id in (match.subject_node_id, match.object_node_id):
                if node_id:
                    if native_authoritative:
                        self._online_match_add_native_authoritative_count += 1
                    else:
                    ...
                    if self._native_online_shadow_enabled():
                        try:
                            ...
                            self._fallback_to_python_online_index(
                                "mutation_failure:register_online_match",
                                rebuild_online_index=True,
                            )
                            native_failed = True
                            break
    ...
    def _disable_native_backend_shadowing(self) -> None:
        if not self._native_online_shadow_enabled():
            self.native_online_read_primary_enabled = False
            self.native_shadow_check_enabled = False
            self.native_online_flush_authoritative_enabled = False
            return
        self.native_online_read_primary_enabled = False
        self.native_shadow_check_enabled = False
        self.native_online_flush_authoritative_enabled = False

    def _fallback_to_python_online_index(self, reason: str, *, rebuild_online_index: bool) -> None:
        self._native_online_fallback_activation_total += 1
        if "mutation" in reason:
            self._native_online_fallback_mutation_failure_total += 1
        elif "mismatch" in reason:
            self._native_online_fallback_read_mismatch_total += 1
        if self._native_online_fallback_first_reason is None:
            self._native_online_fallback_first_reason = str(reason)
        self._disable_native_backend_shadowing()
        if rebuild_online_index:
            self._rebuild_online_index_from_active_matches()

    def _handle_native_read_mismatch(self, reason: str) -> None:
        self._native_online_read_fallback_total += 1
        self._fallback_to_python_online_index(reason, rebuild_online_index=False)

    def _extend_online_edges(self, edges: list[HSGEdge]) -> int:
        added = 0
        for edge in edges:
    ...
    def _remove_hsg_edges_from_online_state(self, edges: list[HSGEdge]) -> int:
        if not edges:
            return 0
        edge_keys = {(edge.src, edge.dst, edge.relation) for edge in edges}
        if not edge_keys:
            return 0
        before = len(self.hsg_edges)
        if before == 0:
            return 0
        self.hsg_edges = [edge for edge in self.hsg_edges if (edge.src, edge.dst, edge.relation) not in edge_keys]
        removed_count = before - len(self.hsg_edges)
        if removed_count <= 0:
            return 0
        self.seen_edges = {(edge.src, edge.dst, edge.relation) for edge in self.hsg_edges}
        self._graph_path_edges_count = len([edge for edge in self.hsg_edges if edge.relation == "graph_path"])
        return removed_count

    def _remove_active_matches_from_online_state(self, match_ids: set[str]) -> int:
        if not match_ids:
            return 0
        affected_matches = [self.match_by_id.get(match_id) for match_id in match_ids]
        affected_matches = [match for match in affected_matches if match is not None]
        if not affected_matches:
            return 0

        # Local online-index delete for Python path.
        native_remove_failed = False
        native_authoritative = self._native_online_match_authoritative_enabled_for_run()
        for match in affected_matches:
            for node_id in (match.subject_node_id, match.object_node_id):
                if node_id:
                    if native_authoritative:
                        self._online_match_remove_native_authoritative_count += 1
                    else:
                        self.online_index.on_match_removed(node_id, match.match_id)
                    if self._native_online_shadow_enabled():
                        try:
                            removed_native = self.native_backend.remove_online_match(node_id, match.match_id)
                        except Exception:
                            removed_native = False
                        if not removed_native:
                            native_remove_failed = True
                            break
            if self._native_online_shadow_enabled() is False and (
                self.native_online_read_primary_enabled is False and self.native_shadow_check_enabled is False
            ):
                break

        removed_ids = {match.match_id for match in affected_matches}
        self.matches = [match for match in self.matches if match.match_id not in removed_ids]
        for match in affected_matches:
            self.match_by_id.pop(match.match_id, None)
            self.hsg_nodes.pop(match.match_id, None)
            entities = self.match_to_entities.pop(match.match_id, set())
            for entity in entities:
                match_bucket = self.node_to_matches.get(entity)
                if match_bucket is not None:
                    match_bucket.discard(match.match_id)
                    if not match_bucket:
                        self.node_to_matches.pop(entity, None)
                hsg_bucket = self.entity_to_hsg_node.get(entity)
                if hsg_bucket is not None:
                    hsg_bucket.discard(match.match_id)
                    if not hsg_bucket:
                        self.entity_to_hsg_node.pop(entity, None)

        self.hsg_edges = [edge for edge in self.hsg_edges if edge.src not in removed_ids and edge.dst not in removed_ids]
        self.seen_edges = {(edge.src, edge.dst, edge.relation) for edge in self.hsg_edges}
        self._graph_path_edges_count = len([edge for edge in self.hsg_edges if edge.relation == "graph_path"])
        if native_remove_failed:
            self._fallback_to_python_online_index(
                "mutation_failure:remove_online_match",
                rebuild_online_index=True,
            )
    ...
    def _rebuild_online_index_from_active_matches(self) -> None:
        ...
        if not self._native_online_flush_authoritative_enabled_for_run():
    ...
    def _online_mapper_match_ids(self, node_id: str) -> set[str]:
        ...
        if self.native_shadow_check_enabled:
            ...
            if python_ids != native_ids:
                self._handle_native_read_mismatch("read_mismatch:match_ids")
                return python_ids
        return native_ids
    ...
    def _online_mapper_contains_rule(self, node_id: str, rule_id: str) -> bool:
        ...
        if self.native_shadow_check_enabled:
            ...
            if python_value != native_value:
                self._handle_native_read_mismatch("read_mismatch:contains_rule")
                return python_value
        return native_value
    ...
    def _online_mapper_earliest_seq(self, node_id: str, rule_id: str) -> int | None:
        ...
        if native_value is None:
            self._handle_native_read_mismatch("read_mismatch:earliest_seq_none")
            return python_value
        if self.native_shadow_check_enabled:
            self._run_native_shadow_check_query_typed("earliest_seq", python_value, native_value)
            if python_value != native_value:
                self._handle_native_read_mismatch("read_mismatch:earliest_seq")
                return python_value
        return native_value
    ...
    def _online_mapper_contains_match(
        ...
        *,
        ...
        if self.native_shadow_check_enabled:
            ...
            if python_value != native_value:
                self._handle_native_read_mismatch("read_mismatch:contains_match")
                return python_value
        return native_value
    ...
    def _online_mapper_min_hops(
        ...
        *,
        ...
        native_value = self.native_backend.online_mapper_min_hops(node_id, match_id, origin_node_id)
        if native_value is None:
            self._handle_native_read_mismatch("read_mismatch:min_hops_none")
            return python_value
        if self.native_shadow_check_enabled:
            self._run_native_shadow_check_query_typed("min_hops", python_value, native_value)
            if python_value != native_value:
                self._handle_native_read_mismatch("read_mismatch:min_hops")
                return python_value
        return native_value

    def _protected_graph_entities(self) -> set[str]:
        # Entity-level protection is intentionally narrow because it prevents
        # pruning the current version of the entire entity lineage.
    ...
    def _protected_graph_version_nodes(self) -> set[str]:
        ...
        for match in self.matches:
            ...
            for entity_id in match.entities:
                if isinstance(entity_id, str):
                    node_id = self.graph.current_version_node(entity_id)
                    if node_id:
                        protected.add(node_id)
            for entity_id in match.bindings.values():
                if isinstance(entity_id, str):
                    node_id = self.graph.current_version_node(entity_id)
                    if node_id:
                        protected.add(node_id)
        for pending in self.hsg_builder.pending_matches_by_id.values():
            ...
            for entity_id in pending.entities:
                if isinstance(entity_id, str):
                    node_id = self.graph.current_version_node(entity_id)
                    if node_id:
                        protected.add(node_id)
            for entity_id in pending.bindings.values():
                if isinstance(entity_id, str):
                    node_id = self.graph.current_version_node(entity_id)
                    if node_id:
                        protected.add(node_id)
    ...
    def _run_graph_deep_gc(self, watermark_ts: str | None, *, force: bool = False) -> None:
        if self.graph_retention_days <= 0:
            return
        cap_pressure = self._graph_gc_cap_pressure()
        if not force and not cap_pressure and (self.stats.events - self._last_graph_gc_events) < self._current_graph_gc_every_events:
            return
        if cap_pressure and not force:
            self._graph_gc_cap_trigger_total += 1
        if not force and not cap_pressure and not self._retention_window_reached(watermark_ts):
            self._last_graph_gc_events = int(self.stats.events)
            self.stats.pruned_event_meta_count += int(self._prune_event_meta_store())
            self._update_graph_gc_cadence(graph_pruned={}, taint_pruned={})
            return
        protected_entities = self._protected_graph_entities()
        protected_version_nodes = self._protected_graph_version_nodes()
        taint_pruned = self.taint_tracker.evict_stale(
            watermark_ts=watermark_ts,
            retention_seconds=self.taint_retention_days * 24 * 60 * 60,
            protected_entities=protected_entities,
            protected_version_nodes=protected_version_nodes,
        )
        self.stats.taint_pruned_entity_count += int(taint_pruned.get("entities_removed", 0))
        self.stats.taint_pruned_version_node_count += int(taint_pruned.get("version_nodes_removed", 0))
        ...
        pruned = self.graph.prune_stale_orphaned(
            ...
            protected_entities=protected_entities,
            protected_version_nodes=protected_version_nodes,
            max_version_nodes=self.graph_max_version_nodes,
            max_edges=self.graph_max_edges,
            cap_low_watermark_ratio=self.graph_prune_low_watermark_ratio,
        )
    ...
    def _graph_gc_cap_pressure(self) -> bool:
        ratio = float(self.graph_gc_cap_pressure_ratio)
        if ratio <= 0.0:
            return False
        if self.graph_max_version_nodes > 0 and len(self.graph.version_nodes) >= max(1, int(self.graph_max_version_nodes * ratio)):
            return True
        if self.graph_max_edges > 0 and len(self.graph.edges) >= max(1, int(self.graph_max_edges * ratio)):
            return True
        return False

    def _retention_window_reached(self, watermark_ts: str | None) -> bool:
        watermark_dt = self.graph._parse_ts(watermark_ts)  # noqa: SLF001
        if watermark_dt is None:
            return True
        earliest_dt = self._earliest_event_dt
        if earliest_dt is None:
            return True

        retention_candidates: list[int] = []
        if self.graph_retention_days > 0:
            retention_candidates.append(self.graph_retention_days * 24 * 60 * 60)
        if self.taint_retention_days > 0:
            retention_candidates.append(self.taint_retention_days * 24 * 60 * 60)
        if not retention_candidates:
            return False

        oldest_relevant_retention_seconds = min(retention_candidates)
        return (watermark_dt - earliest_dt).total_seconds() >= float(oldest_relevant_retention_seconds)

    def _update_graph_gc_cadence(
        self,
        *,
        graph_pruned: dict[str, int],
        taint_pruned: dict[str, int],
    ) -> None:
        removed_total = 0
        for payload in (graph_pruned, taint_pruned):
            removed_total += int(payload.get("entities_removed", 0))
            removed_total += int(payload.get("version_nodes_removed", 0))
            removed_total += int(payload.get("edges_removed", 0))
            removed_total += int(payload.get("semantic_edges_removed", 0))

        if self._graph_gc_cap_pressure():
            self._current_graph_gc_every_events = max(1, self.graph_gc_every_events)
            return

        if removed_total <= 0:
            self._current_graph_gc_every_events = min(
                int(self.graph_gc_max_every_events),
                max(1, int(self._current_graph_gc_every_events) * 2),
            )
            return

        if self._current_graph_gc_every_events > self.graph_gc_every_events:
            self._current_graph_gc_every_events = max(
                int(self.graph_gc_every_events),
                int(self._current_graph_gc_every_events) // 2,
            )
    ...
    def _native_online_shadow_enabled(self) -> bool:
        return bool(
            self.native_backend.available and (self.native_online_read_primary_enabled or self.native_shadow_check_enabled)
        )

    def _native_online_flush_authoritative_enabled_for_run(self) -> bool:
        return bool(
            self.native_online_flush_authoritative_enabled
            and self.native_backend.available
            and self.native_online_read_primary_enabled
            and not self.native_shadow_check_enabled
        )

    def _native_online_match_authoritative_enabled_for_run(self) -> bool:
        return self._native_online_flush_authoritative_enabled_for_run()

    def _flush_pending_online_graph_edges(self) -> None:
        if not self.use_online_prereq or not self._pending_online_graph_edges:
            return
        started = time.perf_counter()
        self._online_graph_edge_flush_call_count += 1
        pending = self._pending_online_graph_edges
        self._pending_online_graph_edges = []
        native_add_failed = False
        for src, dst, edge_type in pending:
            python_apply_started = time.perf_counter()
            self.online_index.on_edge_added(src, dst, edge_type, propagate=False)
            self._online_graph_edge_flush_python_apply_time_seconds += time.perf_counter() - python_apply_started
            if self._native_online_shadow_enabled():
                native_add_started = time.perf_counter()
                try:
                    self.native_backend.add_online_edge(src, dst, self._native_edge_type_name(edge_type))
                except Exception:
                    native_add_failed = True
                    self._online_graph_edge_flush_native_add_time_seconds += time.perf_counter() - native_add_started
                    break
                self._online_graph_edge_flush_native_add_time_seconds += time.perf_counter() - native_add_started
        if native_add_failed:
            self._fallback_to_python_online_index(
                "mutation_failure:add_online_edge",
                rebuild_online_index=False,
            )
            self.online_index.flush_pending_edges()
            self._online_graph_edge_flush_time_seconds += time.perf_counter() - started
            return
        flush_call_started = time.perf_counter()
        if self._native_online_flush_authoritative_enabled_for_run():
            self._online_graph_edge_flush_native_authoritative_count += 1
        else:
            self.online_index.flush_pending_edges()
        if self._native_online_shadow_enabled():
            self.native_backend.flush()
        self._online_graph_edge_flush_flush_call_time_seconds += time.perf_counter() - flush_call_started
        self._online_graph_edge_flush_time_seconds += time.perf_counter() - started

    def _maybe_flush_pending_online_graph_edges(
        self,
        *,
        ...
        if not self.use_online_prereq or not self._pending_online_graph_edges:
            return
        if force:
            self._online_graph_edge_flush_forced_count += 1
            self._flush_pending_online_graph_edges()
            return
        if self.online_graph_edge_flush_policy == "immediate":
            self._flush_pending_online_graph_edges()
            return
        if raw_match_count > 0:
            self._online_graph_edge_flush_match_driven_count += 1
            self._flush_pending_online_graph_edges()
            return
        if len(self._pending_online_graph_edges) >= self.online_graph_edge_flush_max_pending_edges:
            self._online_graph_edge_flush_threshold_count += 1
            self._flush_pending_online_graph_edges()
            return
        self._online_graph_edge_flush_skipped_count += 1
    ...
    def process_event(self, event: Event) -> None:
        ...
        event_dt = self.graph._parse_ts(event.ts)  # noqa: SLF001
        if event_dt is not None and (self._earliest_event_dt is None or event_dt < self._earliest_event_dt):
            self._earliest_event_dt = event_dt
        ...
        if node_info is None:
            ...
            self._maybe_flush_pending_online_graph_edges(force=True)
            graph_elapsed += time.perf_counter() - flush_started
            self._graph_add_time_seconds += graph_elapsed
            return
        ...
        flush_started = time.perf_counter()
        self._maybe_flush_pending_online_graph_edges(raw_match_count=len(raw_matches))
        graph_elapsed += time.perf_counter() - flush_started
        self._graph_add_time_seconds += graph_elapsed
    ...
    def process_event_with_precomputed_matches(
        ...
        flush_started = time.perf_counter()
        self._maybe_flush_pending_online_graph_edges(raw_match_count=len(effective_raw_matches))
        graph_elapsed += time.perf_counter() - flush_started
        self._graph_add_time_seconds += graph_elapsed
    ...
    def _apply_event_matches(
        ...
        *,
        ...
        self._hsg_update_call_count += 1
        event_changed_match_count = 0
        event_changed_edge_count = 0
            ...
            if self.use_online_prereq:
                ...
                if has_prereq and not satisfied:
                    ...
                    if accepted:
                        ...
                        added_active_match = self._add_active_match_to_online_state(new_match, add_to_online_index=True)
                        self._online_add_active_match_time_seconds += time.perf_counter() - active_add_started
                        if added_active_match:
                            event_changed_match_count += 1
                        ...
                        evicted_hsg_edges = list(self.hsg_builder.last_evicted_hsg_edges)
                        ...
                        for activated_match_id in sorted(activated_match_ids):
                            ...
                            if activated_match is not None:
                                active_add_started = time.perf_counter()
                                added_active_match = self._add_active_match_to_online_state(
                                    activated_match,
                                    add_to_online_index=True,
                                )
                                self._online_add_active_match_time_seconds += time.perf_counter() - active_add_started
                                if added_active_match:
                                    event_changed_match_count += 1
                        extend_started = time.perf_counter()
                        event_changed_edge_count += self._extend_online_edges(built_edges)
                        self._online_extend_edges_time_seconds += time.perf_counter() - extend_started
                        local_removed = 0
                        if closed_match_ids:
                            local_removed = self._remove_active_matches_from_online_state(set(closed_match_ids))
                            event_changed_match_count += local_removed
                        local_edges_removed = 0
                        if evicted_hsg_edges:
                            local_edges_removed = self._remove_hsg_edges_from_online_state(evicted_hsg_edges)
                            event_changed_edge_count += local_edges_removed
                        if local_removed or local_edges_removed:
                            self._sync_online_state_from_builder()
                        elif evicted_hsg_edges_count > 0:
                            # HSG edge eviction alone should not force an online-index rebuild.
                            # Fall back to a cheap HSG state sync only when the local edge patch-up missed.
                            self._sync_online_state_from_builder()
                    else:
        ...
        self._hsg_update_changed_match_count += event_changed_match_count
        self._hsg_update_changed_edge_count += event_changed_edge_count
    ...
    def _build_summary(self, *, hsg_nodes_count: int, hsg_edges_count: int) -> dict[str, Any]:
        ...
        graph_gc_telemetry = {
            ...
            "max_version_nodes": int(self.graph_max_version_nodes),
            "max_edges": int(self.graph_max_edges),
            "taint_retention_days": int(self.taint_retention_days),
            ...
            "taint_entities_removed": int(self.stats.taint_pruned_entity_count),
            "taint_version_nodes_removed": int(self.stats.taint_pruned_version_node_count),
    ...
    def write_snapshot(self, out_dir: str | Path) -> dict[str, Any]:
        ...
        result = {
            "summary": summary,
            "matches": list(self._iter_match_rows()),
            "hsg": hsg_to_dict(HSG(nodes=list(self.hsg_nodes.values()), edges=list(self.hsg_edges))),
        }
