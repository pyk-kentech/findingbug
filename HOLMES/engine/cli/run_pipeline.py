...
def run_pipeline(
    ...
    max_hsg_edges: int = 0,
    scenario_dormancy_days: int | None = None,
    ...
    if quality_variant == "b":
        ...
        effective_scenario_dormancy_days = max(0, int(scenario_dormancy_days)) if scenario_dormancy_days is not None else 7
        effective_graph_retention_days = max(0, int(graph_retention_days)) if graph_retention_days is not None else 14
    else:
        ...
        effective_scenario_dormancy_days = max(0, int(scenario_dormancy_days)) if scenario_dormancy_days is not None else 60
        effective_graph_retention_days = max(0, int(graph_retention_days)) if graph_retention_days is not None else 60
    ...
    engine = StreamingEngine(
        ...
        scenario_dormancy_days=effective_scenario_dormancy_days,
        graph_retention_days=effective_graph_retention_days,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="scenario_dormancy_days",
        type=int,
        default=None,
        help="Close dormant active scenarios after N days without activity (default: A=60, B=7).",
    )
    parser.add_argument(
        "--graph-retention-days",
        dest="graph_retention_days",
        type=int,
        default=None,
        help="Retain raw graph entities/version nodes for N days before prune (default: A=60, B=14).",
    )
    parser.add_argument(
        "--matcher-workers",
...
def main() -> int:
    ...
    if args.train_events:
        ...
        run_pipeline(
            ...
            scenario_dormancy_days=(None if args.scenario_dormancy_days is None else max(0, int(args.scenario_dormancy_days))),
            graph_retention_days=(None if args.graph_retention_days is None else max(0, int(args.graph_retention_days))),
