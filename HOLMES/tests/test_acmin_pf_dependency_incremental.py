...
def test_prev_version_fast_path_preserves_ancestor_index():
    g = ProvenanceGraph()
    first = g.add_event(Event(event_id="e1", ts=None, event_type="write", subject="proc:P", object="file:F", raw={}))
    second = g.add_event(Event(event_id="e2", ts=None, event_type="write", subject="proc:P", object="file:F", raw={}))

    assert first is not None and second is not None
    v1 = first["object_node_id"]
    v2 = second["object_node_id"]
    proc_v = second["subject_node_id"]

    assert g._min_dist_from_ancestor[v2][v2] == 0  # noqa: SLF001
    assert g._min_dist_from_ancestor[v2][v1] == 0  # noqa: SLF001
    assert g._min_dist_from_ancestor[v2][proc_v] == 1  # noqa: SLF001
    assert g.path_factor(v1, v2) == 1.0
