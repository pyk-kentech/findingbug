...
def test_data_flow_precheck_can_be_disabled_via_env(monkeypatch):
    monkeypatch.setenv("HOLMES_ONLINE_INDEX_ENABLE_DATA_FLOW_PRECHECK", "0")
    ...
    assert "T1" in idx.mapper_match_ids("v")
    assert idx.propagate_across_new_edge_data_flow_precheck_count == 0
    assert idx.propagate_across_new_edge_data_flow_precheck_hit_count == 0


def test_data_flow_precheck_can_be_enabled_via_env(monkeypatch):
    monkeypatch.setenv("HOLMES_ONLINE_INDEX_ENABLE_DATA_FLOW_PRECHECK", "1")

    idx = OnlineIndex()
    idx.on_match_added("u", ttp_id="T1", sequence=1)
    idx.on_edge_added("u", "v", EdgeType.DATA_FLOW)

    idx.on_match_added("x", ttp_id="T2", sequence=2)
    idx.on_edge_added("x", "v", EdgeType.DATA_FLOW)

    assert "T2" in idx.mapper_match_ids("v")
    assert idx.propagate_across_new_edge_data_flow_precheck_count >= 1
