...
class Matcher:
    ...
    @staticmethod
    def _get_field_value(raw: dict[str, Any], field: str) -> Any:
        def lookup_case_insensitive(current: Any, key_name: str) -> Any:
        ...
        def get_path_value(current: Any, parts: list[str]) -> Any:
            ...
            if part.endswith("{}"):
                list_field = part[:-2]
                list_value = lookup_case_insensitive(current, list_field)
            ...
            next_value = lookup_case_insensitive(current, part)
            if next_value is None:
                return None
            return get_path_value(next_value, rest)

        if "." not in field and not field.endswith("{}"):
            return lookup_case_insensitive(raw, field)
    ...
    @classmethod
    def _evaluate_field_match(cls, event: Event, predicate: dict[str, Any]) -> bool:
        ...
        field_values = cls._string_values(cls._get_field_value(event.raw, field_name))
    ...
    @classmethod
    def _evaluate_sigma_selector(cls, event: Event, selector: Any) -> bool:
        ...
        for item in items:
            if not isinstance(item, dict):
                return False
            if item.get("type") != "field_match":
                return False
            if not cls._evaluate_field_match(event, item):
                return False
        return True
    ...
    def _evaluate_sigma_condition(
        ...
        if node_type == "selector_ref":
            ...
            if not isinstance(selector_name, str):
                return False
            return cls._evaluate_sigma_selector(event, selectors.get(selector_name))
        if node_type == "selector_group":
            ...
            matched = sum(
                ...
                and cls._evaluate_sigma_selector(event, selectors.get(selector_name))
            )
        ...
        if node_type == "logical":
            ...
            if not isinstance(operands, list):
                return False
            evaluated = [cls._evaluate_sigma_condition(event, operand, selectors) for operand in operands]
            if operator == "AND":
    ...
    @classmethod
    def _sigma_match(cls, rule: Rule, event: Event) -> bool:
        ...
        if not isinstance(selectors, dict):
            return False
        condition_payload = logic.get("condition", {})
        if not isinstance(condition_payload, dict):
            return False
        compiled = condition_payload.get("compiled")
        return cls._evaluate_sigma_condition(event, compiled, selectors)
    ...
    def _entity_from_raw_field(
        ...
        for field_name in field_names:
            ...
            for string_value in cls._string_values(value):
                ...
                if graph.current_version_node(entity_id) is None:
                    return None, {
            ...
            for event in events:
                ...
                if rule.event_predicate:
                    ...
                    if expected_event_type is not None and ev_event_type != expected_event_type:
                        continue
                elif not self._sigma_match(rule, event):
                    continue

                if self._should_use_entity_type_filters(rule):
                    ...
                    event_source_types = self._event_source_types(event)
                    allowed_source_types = {x.lower() for x in rule.source_types}
                    if event_source_types and not (allowed_source_types & event_source_types):
                        continue
